...
9. If the user has additions, acknowledge them, adjust the estimated total if needed, and continue with only the questions needed to confirm the new information. If the user explicitly has no additions or asks to proceed, say that requirements are confirmed and start the originally requested implementation or planning output.
10. Generate a PRD only when the user explicitly requests a PRD after confirmation. Create `PRD-[product-name].md` with these sections: Summary, Contacts, Background, Objective, Market Segment(s), Value Proposition(s), Solution, and Release. Use confirmed information, mark missing facts as `TBD`, and do not restart discovery.
11. Do not automatically create a PRD. Without an explicit PRD request, carry the confirmed requirements into the user's authorized task.
...
After confirmation, summarize only the confirmed requirements and remaining open risks, then proceed with the authorized task. Generate a PRD only when the user explicitly requests it.
